...
class Test__compare_name_lists:
    ...
    def test_difforder_tolerant_warns(self):
        errs = []
        with pytest.warns(UserWarning, match="Ignoring: named-elements do not match"):
            _compare_name_lists(
                errs, ['a', 'b'], ['b', 'a'], 'named-elements',
                order_strict=False
            )
        assert errs == []
    ...
    def test_difforder_tolerant_nowarn(self):
        ...
        with warnings.catch_warnings():
            ...
            _compare_name_lists(
                errs, ['a', 'b'], ['b', 'a'], 'named-elements',
                order_strict=False, suppress_warnings=True
            )
        assert errs == []
...
class Test__compare_attributes:
    ...
    def test_compare_attributes_values__data_mismatch(self):
        ...
        errs = []
        _compare_attributes(errs, obj1, obj2, '<object attributes>')
        assert errs == ['<object attributes> "b" attribute values differ : 2 != -77']
    ...
    def test_compare_attributes_values__dtype_and_data_mismatch(self):
        ...
        errs = []
        _compare_attributes(errs, obj1, obj2, '<object attributes>')
        assert errs == ['<object attributes> "a" attribute values differ : 0.0 != 1.0']
